            ...
            if ($cek_akun_status > 0) {
                ...
                if ($cek > 0) {
                    if ($cek != 1) {
                        ...
                        $ex_compid = DB::tabel('m_user_company')->select('company_id', 'nama_usaha', 'alamat')->where($sub)->orWhere($query);
                        ...
                        $sub = DB::table('m_userx')->select('id')->where('email', '=', $data['input'])->Where('passwd', '=', $data['passwd']);
                        $query = DB::table('m_userx')->select('id')->where('no_hp', '=', $data['input'])->Where('passwd', '=', $data['passwd']);
                        $ex_compid = DB::tabel('m_user_company')->select('company_id', 'nama_usaha', 'alamat')->where($sub)->orWhere($query)->get();
                       return $ex_compid;
                    }
                }else {
